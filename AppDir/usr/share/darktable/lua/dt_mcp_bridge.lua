...
local function create_ai_panel()
    ...
    local function ensure_server_running()
        ...
        if f then
            f:close()
             -- If file exists, check timestamp? For now assume running if file exists.
             -- But file might be stale.
             running = true
        end

        if not running then
            status_cloud.label = "Starting AI Server..."
            
            -- Try to find the python script
            -- 1. User config (Manual install)
            -- 2. System install (implied by this integration)
            
            local script_locations = {
                dt.configuration.config_dir .. "/mcp/dt_mcp_server.py",
                "/usr/share/darktable/mcp/dt_mcp_server.py",
                "/usr/local/share/darktable/mcp/dt_mcp_server.py"
            }
            
            local script_path = nil
            for _, p in ipairs(script_locations) do
            ...
            if script_path then
                dt.print_log("[MCP] Starting server from " .. script_path)
                -- Run in background using nohup or &
                -- Note: dt.control.execute waits? We need async.
                -- Using os.execute with & might work on Linux.
                os.execute("python3 " .. script_path .. " > /dev/null 2>&1 &")
                
                -- Wait a bit for startup
                dt.print_log("[MCP] Server launch command sent.")
            else
                status_cloud.label = "Error: Server Script Not Found"
                dt.print_log("[MCP] Server script not found in search paths")
            end
        end
        
        -- Schedule a real status check
        dt.register_event("mcp_startup_check", 2000, function()
             check_status()
        end)
    end
...
local widget = create_ai_panel()

if dt.gui.views.mcp and dt.gui.views.mcp.set_widget then
    dt.gui.views.mcp:set_widget(widget)
else
    dt.print_log("[MCP] MCP view not found, falling back to Lighttable panel")
    dt.register_lib("mcp_ai_assistant", "MCP AI Assistant", true, false, {[dt.gui.views.lighttable] = {"DT_UI_CONTAINER_PANEL_RIGHT_CENTER", 100}}, widget)
end
