...
static void usage(const char *progname) {
  fprintf(
      ...
      "                     use --help icc-intent for list of supported "
      "intents\n"
      "   --verbose\n"
      "   -h, --help [option]\n"
      "   -v, --version\n",
...
static void icc_types() {
  // Ensure that if new colorspaces are added, this list is updated.
  // DT_COLORSPACE_LAST is 27 (0 to 26 covers FILE to DISPLAY_P3).
  _Static_assert(DT_COLORSPACE_LAST == 27,
                 "src/cli/main.c: icc_types() list needs update, "
                 "DT_COLORSPACE_LAST changed");
  ...
  fprintf(stderr, " BRG\n");
  fprintf(stderr, " EXPORT\n"); // export and softproof are categories and will
                                // return NULL with dt_colorspaces_get_profile()
...
static dt_colorspaces_color_profile_type_t get_icc_type(const char *option) {
  ...
  ICC_FROM_STR(BRG);
  ICC_FROM_STR(EXPORT); // export and softproof are categories and will return
                        // NULL with dt_colorspaces_get_profile()
...
static void icc_intents() {
  // Ensure that if new intents are added, this list is updated.
  _Static_assert(DT_INTENT_LAST == 4, "src/cli/main.c: icc_intents() list "
                                      "needs update, DT_INTENT_LAST changed");
...
int main(int argc, char *arg[]) {
  ...
  for (k = 1; k < argc; k++) {
    if (arg[k][0] == '-') {
      if (!strcmp(arg[k], "--help") || !strcmp(arg[k], "-h")) {
        ...
        printf("darktable %s\nCopyright (C) 2012-%s Johannes Hanika and other "
               "contributors.\n\n",
               darktable_package_version, darktable_last_commit_year);
        ...
        fprintf(stderr, "%s %d\n",
                _("TODO: sorry, due to API restrictions we currently cannot "
                  "set the BPP to"),
                bpp);
        ...
        if (!g_strcmp0(str, "0") || !g_strcmp0(str, "FALSE"))
          ...
          fprintf(stderr, "%s: %s\n", _("unknown option for --export_masks"),
                  arg[k]);
          usage(arg[0]);
          exit(1);
        }
        ...
        if (g_file_test(arg[k], G_FILE_TEST_EXISTS))
          ...
          fprintf(stderr,
                  _("notice: input file or dir '%s' doesn't exist, skipping\n"),
                  arg[k]);
  ...
  if ((inputs && file_counter < 1) || (!inputs && file_counter < 2) ||
      file_counter > 3) {
    usage(arg[0]);
    free(m_arg);
    if (output_filename)
    ...
    fprintf(stderr, _("error: input file and import opts specified! that's not "
                      "supported!\n"));
    usage(arg[0]);
    free(m_arg);
    if (output_filename)
  ...
  if (g_file_test(output_filename, G_FILE_TEST_IS_DIR)) {
    ...
    fprintf(stderr,
            _("notice: output location is a directory. assuming "
              "'%s/$(FILE_NAME).%s' output pattern"),
  ...
  if (g_file_test(output_filename, G_FILE_TEST_EXISTS) && !output_to_dir) {
    if (!output_ext ||
        (output_ext && g_str_has_suffix(output_filename, output_ext) &&
         !g_strcmp0(output_ext, strrchr(output_filename, '.') + 1))) {
      // output file exists or there's output ext specified and it's same as
      // file...
      fprintf(stderr, "%s\n",
      ...
      if (cdir) {
        ...
        while ((fname = g_dir_read_name(cdir)) != NULL) {
          ...
          if (!g_file_test(fullname, G_FILE_TEST_IS_DIR) &&
              dt_supported_image(fname)) {
            // Import each supported image file directly
            const dt_imgid_t imgid =
                dt_image_import(filmid, fullname, TRUE, TRUE);
  ...
  // print the history stack. only look at the first image and assume all got
  // the same processing applied
  if (verbose) {
  ...
  if (!output_ext) {
    // by this point we're sure output is not dir, there's no output ext
    // specified so only place to look for it is in filename try to find out the
    // export format from the output_filename
  ...
  storage = dt_imageio_get_storage_by_name(
      "disk"); // only exporting to disk makes sense
  if (storage == NULL) {
    fprintf(stderr, "%s\n",
            _("cannot find disk storage module. please check your "
              "installation, something seems to be broken."));
    free(m_arg);
    g_free(output_filename);
    g_free(output_ext);
    exit(1);
  ...
  // and now for the really ugly hacks. don't tell your children about this one
  // or they won't sleep at night any longer ...
  ...
  fdata->style[0] = '\0';
  fdata->style_append =
      1; // make append the default and override with --style-overwrite
  ...
  if (storage->initialize_store) {
    storage->initialize_store(storage, sdata, &format, &fdata, &id_list,
                              high_quality, upscale);
  ...
  for (GList *iter = id_list; iter; iter = g_list_next(iter), num++) {
    ...
    if (custom_presets) {
      ...
      metadata.list =
          dt_util_str_to_glist("\1", dt_lib_export_metadata_get_conf());
      if (metadata.list)
    ...
    if (storage->store(storage, sdata, id, format, fdata, num, total,
