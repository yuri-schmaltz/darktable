...
/** Note :
 * we use finite-math-only and fast-math because divisions by zero are manually
 * avoided in the code fp-contract=fast enables hardware-accelerated Fused
 * Multiply-Add
 **/
#if defined(__GNUC__)
#pragma GCC optimize("finite-math-only", "fp-contract=fast", "fast-math",      \
                     "no-math-errno")
#endif
...
// These limits clamp away insane memory requirements.  They should reasonably
// faithfully represent the full precision though, so tiling will help reduce
// the memory footprint and export will look the same as darkroom mode (only
// 1mpix there).
#define DT_COMMON_BILATERAL_MAX_RES_S 3000
#define DT_COMMON_BILATERAL_MAX_RES_R 50

void dt_bilateral_grid_size(dt_bilateral_t *b, const int width,
                            const int height, const float L_range,
  ...
  float _x =
      CLAMPS((int)roundf(width / sigma_s), 4, DT_COMMON_BILATERAL_MAX_RES_S);
  float _y =
      CLAMPS((int)roundf(height / sigma_s), 4, DT_COMMON_BILATERAL_MAX_RES_S);
  float _z =
      CLAMPS((int)roundf(L_range / sigma_r), 4, DT_COMMON_BILATERAL_MAX_RES_R);
  ...
  // cache the reciprocals so that we can multiply instead of dividing to get a
  // grid point
...
size_t dt_bilateral_memory_use(
...
#else
  return (grid_size + 3 * dt_get_num_threads() * b.size_x * b.size_z) *
         sizeof(float);
#endif /* HAVE_OPENCL */
}
...
size_t dt_bilateral_memory_use2(const int width, const int height,
...
// for the CPU path this is just an alias as no additional temp buffer is needed
// when compiling with OpenCL, version in bilateralcl.c takes precedence
size_t dt_bilateral_singlebuffer_size2(const int width, const int height,
                                       const float sigma_s,
                                       const float sigma_r) {
...
static size_t image_to_grid(const dt_bilateral_t *const b, const int i,
                            const int j, const float L, float *xf, float *yf,
                            float *zf) {
...
static size_t image_to_relgrid(const dt_bilateral_t *const b, const int i,
                               const float L, float *xf, float *zf) {
  float x = CLAMPS(i * b->sigma_s_inv, 0, b->size_x - 1);
  float z = CLAMPS(L * b->sigma_r_inv, 0, b->size_z - 1);
  const int xi = MIN((int)x, b->size_x - 2);
  const int zi = MIN((int)z, b->size_z - 2);
...
dt_bilateral_t *
dt_bilateral_init(const int width,     // width of input image
                  const int height,    // height of input image
                  const float sigma_s, // spatial sigma (blur pixel coords)
                  const float sigma_r) // range sigma (blur luma values)
{
  ...
  b->buf = dt_calloc_align_float(b->size_x * b->size_z * b->numslices *
                                 b->slicerows);
  if (!b->buf) {
  ...
  dt_print(DT_DEBUG_DEV,
           ...
           b->size_x, b->size_y, b->size_z, b->sigma_s, sigma_s, b->sigma_r,
           sigma_r);
  return b;
}
...
void dt_bilateral_splat(const dt_bilateral_t *b, const float *const in) {
  ...
  const size_t offsets[8] = {0,  ox,      oy,      ox + oy,
  ...
  for (int slice = 0; slice < b->numslices; slice++) {
    ...
    const int slice_offset =
        slice * b->slicerows - (int)(firstrow * b->sigma_s_inv);
    ...
    for (int j = firstrow; j < lastrow; j++) {
      ...
      for (int i = 0; i < b->width; i++) {
        ...
        const dt_aligned_pixel_t contrib = {
            ...
            (1.0f - xf) * yf * 100.0f / sigma_s, xf * yf * 100.0f / sigma_s};
...
static void blur_line_z(float *buf, const int offset1, const int offset2,
                        const int offset3, const int size1, const int size2,
                        const int size3) {
  ...
  for (int k = 0; k < size1; k++) {
    ...
    for (int j = 0; j < size2; j++) {
      ...
      buf[index] =
          w1 * (buf[index + offset3] - tmp1) + w2 * buf[index + 2 * offset3];
      index += offset3;
...
DT_OMP_DECLARE_SIMD(aligned(buf : 64))
static void blur_line(float *buf, const int offset1, const int offset2,
                      const int offset3, const int size1, const int size2,
                      const int size3) {
  ...
  for (int k = 0; k < size1; k++) {
    ...
    for (int j = 0; j < size2; j++) {
      ...
      const float tmp3 = buf[index];
      buf[index] =
          buf[index] * w0 + w1 * (buf[index + offset3] + tmp2) + w2 * tmp1;
      index += offset3;
...
void dt_bilateral_slice(const dt_bilateral_t *const b, const float *const in,
                        float *out, const float detail) {
  ...
  for (int j = 0; j < height; j++) {
    for (int i = 0; i < width; i++) {
      ...
      const float Lout = fmaxf(
          0.0f, L + norm * (buf[gi] * (1.0f - xf) * (1.0f - yf) * (1.0f - zf) +
