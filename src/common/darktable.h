...
// needs to be defined before any system header includes for control/conf.h to
// work in C++ code
...
#else
// GCC 8.4 throws string of errors "'x' is predetermined 'shared' for 'shared'"
// if we explicitly declare
...
#else
// GCC7/8 only support OpenMP 4.5, which does not have the nontemporal()
// directive.
 ...
 * paralellized/vectorized loops */
    ...
    !defined(NATIVE_ARCH) && !defined(__APPLE__) && defined(__GLIBC__)
#if defined(__amd64__) || defined(__amd64) || defined(__x86_64__) ||           \
    defined(__x86_64)
#define __DT_CLONE_TARGETS__                                                   \
  __attribute__((target_clones("default", "sse2", "sse3", "sse4.1", "sse4.2",  \
                               "popcnt", "avx", "avx2", "avx512f", "fma4")))
...
/* Helper to force stack vectors to be aligned on DT_CACHELINE_BYTES blocks to
 * enable AVX2 */
...
// bump this number and make sure you have an updated logic in
...
#define DT_MODULE(MODVER)                                                      \
  int dt_module_dt_version() { return -DT_MODULE_VERSION; }                    \
  int dt_module_mod_version() { return MODVER; }
#else
#define DT_MODULE(MODVER)                                                      \
  int dt_module_dt_version() { return DT_MODULE_VERSION; }                     \
  int dt_module_mod_version() { return MODVER; }
...
// This is not only faster but more accurate because rounding happens at the
// right place
#ifdef FP_FAST_FMAF
...
typedef float
    dt_boundingbox_t[4]; //(x,y) of upperleft, then (x,y) of lowerright
...
typedef enum dt_debug_thread_t {
  ...
  DT_DEBUG_COMMON = DT_DEBUG_OPENCL | DT_DEBUG_DEV | DT_DEBUG_MASKS |
                    DT_DEBUG_PARAMS | DT_DEBUG_IMAGEIO | DT_DEBUG_PIPE,
...
int dt_init(int argc, char *argv[], const gboolean init_gui,
            const gboolean load_data, lua_State *L);
...
#define dt_print_nts(thread, ...)                                              \
  dt_debug_if(thread, dt_print_nts_ext, __VA_ARGS__)
#define dt_print_pipe(thread, title, pipe, module, device, roi_in, roi_out,    \
                      ...)                                                     \
  dt_debug_if(thread, dt_print_pipe_ext, title, pipe, module, device, roi_in,  \
              roi_out, " " __VA_ARGS__)
...
void dt_dump_pfm_file(const char *pipe, const void *data, const int width,
                      const int height, const int bpp, const char *modname,
                      const char *head, const gboolean input,
                      const gboolean output, const gboolean cpu);

void dt_dump_pfm(const char *filename, const void *data, const int width,
                 const int height, const int bpp, const char *modname);

void dt_dump_pipe_pfm(const char *mod, const void *data, const int width,
                      const int height, const int bpp, const gboolean input,
                      const char *pipe);

void dt_dump_pipe_diff_pfm(const char *mod, const float *a, const float *b,
...
#define dt_calloc1_align_type(TYPE)                                            \
  ((TYPE *)__builtin_assume_aligned(dt_calloc_aligned(sizeof(TYPE)),           \
                                    DT_CACHELINE_BYTES))
#define dt_calloc_align_type(TYPE, count)                                      \
  ((TYPE *)__builtin_assume_aligned(dt_calloc_aligned(count * sizeof(TYPE)),   \
                                    DT_CACHELINE_BYTES))
static inline int *dt_calloc_align_int(const size_t n_ints) {
  return (int *)__builtin_assume_aligned(
      dt_calloc_aligned(n_ints * sizeof(int)), DT_CACHELINE_BYTES);
}

#define dt_alloc1_align_type(TYPE)                                             \
  ((TYPE *)__builtin_assume_aligned(dt_alloc_aligned(sizeof(TYPE)),            \
                                    DT_CACHELINE_BYTES))

#define dt_alloc_align_type(TYPE, count)                                       \
  ((TYPE *)__builtin_assume_aligned(dt_alloc_aligned(count * sizeof(TYPE)),    \
                                    DT_CACHELINE_BYTES))
...
static inline uint8_t *dt_alloc_align_uint8(const size_t n_ints) {
  return (uint8_t *)__builtin_assume_aligned(
      dt_alloc_aligned(n_ints * sizeof(uint8_t)), DT_CACHELINE_BYTES);
}
static inline float *dt_alloc_align_float(const size_t nfloats) {
  return (float *)__builtin_assume_aligned(
      dt_alloc_aligned(nfloats * sizeof(float)), DT_CACHELINE_BYTES);
}
static inline double *dt_alloc_align_double(const size_t ndoubles) {
  return (double *)__builtin_assume_aligned(
      dt_alloc_aligned(ndoubles * sizeof(double)), DT_CACHELINE_BYTES);
}
...
static inline void dt_unlock_image(const dt_imgid_t imgid)
  ...
  dt_pthread_mutex_unlock(
      &(darktable.db_image[imgid & (DT_IMAGE_DBLOCKS - 1)]));
}
...
static inline void dt_lock_image_pair(const dt_imgid_t imgid1,
  ...
  if (imgid1 < imgid2) {
    dt_pthread_mutex_lock(
        &(darktable.db_image[imgid1 & (DT_IMAGE_DBLOCKS - 1)]));
    dt_pthread_mutex_lock(
        &(darktable.db_image[imgid2 & (DT_IMAGE_DBLOCKS - 1)]));
  } else {
    dt_pthread_mutex_lock(
...
dt_modifier_is(const GdkModifierType state,
...
static inline gboolean
dt_modifiers_include(const GdkModifierType state,
                     const GdkModifierType desired_modifier_mask) {
  // TODO: on Macs, remap the GDK_CONTROL_MASK bit in
  // desired_modifier_mask to be the bit for the Cmd key
  const GdkModifierType modifiers = gtk_accelerator_get_default_mod_mask();
...
static inline gboolean dt_is_aligned(const void *pointer,
...
void dt_show_times_f(const dt_times_t *start, const char *prefix,
                     const char *suffix, ...)
...
static inline dt_hash_t dt_hash(dt_hash_t hash, const void *data,
                                const size_t size) {
  const uint8_t *str = (uint8_t *)data;
  // Scramble bits in str to create an (hopefully) unique hash representing the
  // state of str Dan Bernstein algo v2 http://www.cse.yorku.ca/~oz/hash.html
  // hash should be inited to DT_INITHASH if first run, or from a previous hash
...
static inline void *dt_alloc_perthread(const size_t n, const size_t objsize,
                                       size_t *padded_size) {
  const size_t alloc_size = n * objsize;
  const size_t cache_lines =
      (alloc_size + DT_CACHELINE_BYTES - 1) / DT_CACHELINE_BYTES;
  *padded_size = DT_CACHELINE_BYTES * cache_lines / objsize;
  const size_t total_bytes =
      DT_CACHELINE_BYTES * cache_lines * dt_get_num_threads();
  return __builtin_assume_aligned(dt_alloc_aligned(total_bytes),
                                  DT_CACHELINE_BYTES);
}
static inline void *dt_calloc_perthread(const size_t n, const size_t objsize,
                                        size_t *padded_size) {
...
// Given the buffer and object count returned by dt_alloc_perthread and a thread
// count in 0..dt_get_num_threads(), return a pointer to the indicated thread's
// private buffer.
...
// a few macros and helper functions to speed up certain frequently-used GLib
// operations
...
static inline const GList *g_list_prev_wraparound(const GList *list) {
  ...
  return g_list_previous(list) ? g_list_previous(list)
...
#define dt_unreachable_codepath()                                              \
  dt_unreachable_codepath_with_caller("unreachable", __FILE__, __LINE__,       \
                                      __FUNCTION__)
...
/*
 * Helper functions for transition to gnome style xgettext translation context
 * marking
 *
 ...
 * markups, like
 *
 *    DT_INTENT_SATURATION = INTENT_SATURATION, // $DESCRIPTION: "rendering
 * intent|saturation"
 *
...
static inline gboolean dt_check_gimpmode_ok(const char *mode) {
  return darktable.gimp.mode
             ? !darktable.gimp.error && strcmp(darktable.gimp.mode, mode) == 0
             : FALSE;
}
