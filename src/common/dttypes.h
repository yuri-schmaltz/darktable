...
// uncomment the next line to use something other than NAN to signal an invalid
// color matrix leave commented out for backward compatibility in case some
...
// Configure the size of a CPU cacheline in bytes, floats, and pixels.  On most
// current architectures, a cacheline contains 64 bytes, but Apple Silicon
// (M-series processors) uses 128-byte cache lines.
...
// If this is applied to a struct member and the struct is allocated on the
// heap, then it must be allocated on a 64 byte boundary to avoid crashes or
// undefined behavior because of unaligned memory access.
#define DT_ALIGNED_ARRAY __attribute__((aligned(DT_CACHELINE_BYTES)))
#define DT_ALIGNED_PIXEL __attribute__((aligned(16)))

// utility type to ease declaration of aligned small arrays to hold a pixel (and
// document their purpose)
typedef DT_ALIGNED_PIXEL float dt_aligned_pixel_t[4];

// picker box definition
typedef float dt_pickerbox_t[8];

// a 3x3 matrix, padded to permit SSE instructions to be used for multiplication
// and addition
typedef float DT_ALIGNED_ARRAY dt_colormatrix_t[4][4];

// To be able to vectorize per-pixel loops, we need to operate on all four
// channels, but if the compiler does not auto-vectorize, doing so increases
// computation by 1/3 for a channel which typically is ignored anyway. Select
// the appropriate number of channels over which to loop to produce the fastest
// code.
...
// A function to compute how many pixels each thread should process in a
// parallelized for loop. For very small RoIs on a CPU with lots of threads, the
// last one or two hardware threads can end up without any work, so there needs
// to be a check whether the starting offset exceeds the total number of pixels
// to be processed.
static inline size_t dt_cacheline_chunks(const size_t npixels,
...
// A macro which gives us a configurable shorthand to produce the optimal
// performance when processing all of the channels in a pixel.  Its first
// argument is the name of the variable to be used inside the 'for' loop it
// creates, while the optional second argument is a set of OpenMP directives,
// typically specifying variable alignment. If indexing off of the beginning of
// any buffer allocated with dt's image or aligned allocation functions, the
// alignment to specify is 64; otherwise, use 16, as there may have been an odd
// number of pixels from the start. Sample usage:
...
#define for_each_channel(_var, ...)                                            \
  _DT_Pragma(omp simd __VA_ARGS__) for (size_t _var = 0;                       \
                                        _var < DT_PIXEL_SIMD_CHANNELS; _var++)
...
static inline void transpose_3xSSE(const dt_colormatrix_t input,
...
// transpose and pad a 3x3 matrix into the padded format optimized for
// vectorization
static inline void transpose_3x3_to_3xSSE(const float input[9],
                                          dt_colormatrix_t output) {
  output[0][0] = input[0];
  output[0][1] = input[3];
  output[0][2] = input[6];
...
static inline void repack_double3x3_to_3xSSE(const double input[9],
                                             dt_colormatrix_t output) {
  output[0][0] = input[0];
  output[0][1] = input[1];
  output[0][2] = input[2];
...
// convert a 3x3 matrix into the padded format optimized for vectorization
static inline void pack_3xSSE_to_3x3(const dt_colormatrix_t input,
                                     float output[9]) {
  output[0] = input[0][0];
  output[1] = input[0][1];
  output[2] = input[0][2];
...
static inline void dt_colormatrix_copy(dt_colormatrix_t out,
                                       const dt_colormatrix_t in) {
