...
static void _list_remove_thumb(gpointer user_data) {
  ...
  gtk_container_remove(GTK_CONTAINER(gtk_widget_get_parent(thumb->w_main)),
                       thumb->w_main);
...
static int _thumbs_get_prefs_size(dt_thumbtable_t *table) {
  ...
  const char *txt =
      dt_conf_get_string_const("plugins/lighttable/thumbnail_sizes");
...
static void _thumbs_update_overlays_mode(dt_thumbtable_t *table) {
  ...
  gchar *txt =
      g_strdup_printf("plugins/lighttable/overlays/%d/%d", table->mode, ns);
...
void dt_thumbtable_set_overlays_mode(dt_thumbtable_t *table,
  ...
  gchar *txt = g_strdup_printf("plugins/lighttable/tooltips/%d/%d", table->mode,
                               table->prefs_size);
  ...
  if (over != table->overlays) {
    ...
    txt = g_strdup_printf("plugins/lighttable/overlays/%d/%d", table->mode,
                          table->prefs_size);
...
void dt_thumbtable_set_overlays_block_timeout(dt_thumbtable_t *table,
  ...
  if (!table)
    return;
  gchar *txt =
      g_strdup_printf("plugins/lighttable/overlays_block_timeout/%d/%d",
                      table->mode, table->prefs_size);
...
static dt_thumbnail_t *_thumb_get_at_pos(dt_thumbtable_t *table, int x, int y) {
  for (const GList *l = table->list; l; l = g_list_next(l)) {
    dt_thumbnail_t *th = l->data;
    if (th->x <= x && th->x + th->width > x && th->y <= y &&
        th->y + th->height > y)
      return th;
  }
  ...
  if (table->mode == DT_THUMBTABLE_MODE_FILEMANAGER) {
    ...
    *x -= table->thumb_size;
    if (*x < table->thumbs_area.x) {
      *x = table->thumbs_area.x +
           (table->thumbs_per_row - 1) * table->thumb_size;
      *y -= table->thumb_size;
    }
  }
}
...
static gboolean _compute_sizes(dt_thumbtable_t *table, gboolean force) {
  ...
  if (table->mode == DT_THUMBTABLE_MODE_FILEMANAGER) {
    ...
    if (force || allocation.width != table->view_width ||
      ...
      table->view_height = allocation.height;
      table->thumb_size =
          MIN(table->view_width / table->thumbs_per_row, table->view_height);
    ...
    if (force || allocation.width != table->view_width ||
      ...
      table->rows =
          (table->view_height - table->thumbs_area.y) / table->thumb_size + 1;
      table->center_offset = 0;
      ret = TRUE;
    }
  }
...
static gboolean _thumbtable_update_scrollbars(dt_thumbtable_t *table) {
  ...
  // now we have a space under last images (when the first shown line is fully
  // shown)
  ...
  if (gtk_widget_get_visible(darktable.gui->scrollbars.vscrollbar) &&
    ...
    if (nblines * nw / table->thumbs_per_row >= table->view_height) {
      dt_view_set_scrollbar(darktable.view_manager->current_view, 0, 0, 0, 0,
                            lbefore, 0, maxvalue + 1, pagesize);
  ...
  */
  const float thresh =
      (float)(table->view_width / table->thumbs_per_row) /
          (float)((table->view_width - bar) / table->thumbs_per_row) -
      1.0f;
  dt_view_set_scrollbar(darktable.view_manager->current_view, 0, 0, 0, 0,
                        lbefore, 0, maxvalue - thresh, pagesize);
  table->code_scrolling = FALSE;
...
static int _thumbs_remove_unneeded(dt_thumbtable_t *table, GList **th_invalid) {
  ...
  for (const GList *l = *th_invalid; l; l = g_list_next(l)) {
    dt_thumbnail_t *th = l->data;
    gtk_container_remove(GTK_CONTAINER(gtk_widget_get_parent(th->w_main)),
                         th->w_main);
...
static void _thumb_move_or_create(dt_thumbtable_t *table, GList **th_invalid,
                                  const int imgid, const int rowid,
  ...
  if (!th_invalid || *th_invalid == NULL) {
    ...
    dt_thumbnail_t *thumb = dt_thumbnail_new(
        table->thumb_size, table->thumb_size, IMG_TO_FIT, imgid, rowid,
        table->overlays, DT_THUMBNAIL_CONTAINER_LIGHTTABLE,
        table->show_tooltips, sel);
    ...
    gtk_widget_set_margin_start(
        thumb->w_image_box, gtk_widget_get_margin_start(first->w_image_box));
    ...
    gtk_layout_move(GTK_LAYOUT(table->widget), thumb->w_main, thumb->x,
                    thumb->y);
...
static int _thumbs_load_needed(dt_thumbtable_t *table, GList **th_invalid,
                               dt_thumbnail_t *first, dt_thumbnail_t *last) {
  if (!first || !last)
    return 0;
  ...
  // we save here the last image value as they may change if the last image is
  // invalid
  ...
  if (first->rowid > 1 &&
    ...
    const int nb_to_load =
        space / table->thumb_size + (space % table->thumb_size != 0);
    // clang-format off
    gchar *query = g_strdup_printf(
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
      if (posy < table->view_height) // we don't load invisible thumbs
      {
        ...
        _thumb_move_or_create(table, th_invalid, imgid, rowid, posx, posy, TRUE,
                              selected);
        changed++;
      }
      _pos_get_previous(table, &posx, &posy);
  ...
  // if the last line is not full, we have already reached the end of the
  // collection
  if ((table->mode == DT_THUMBTABLE_MODE_FILEMANAGER &&
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
      if (posy + table->thumb_size > 0) // we don't load invisible thumbs
      {
        ...
        const int rowid = sqlite3_column_int(stmt, 0);
        const gboolean selected = (imgid == sqlite3_column_int(stmt, 2));
        _thumb_move_or_create(table, th_invalid, imgid, rowid, posx, posy,
                              FALSE, selected);
        changed++;
      }
      _pos_get_next(table, &posx, &posy);
...
static gboolean _move(dt_thumbtable_t *table, const int x, const int y,
  ...
  if (clamp) {
    ...
    if (table->mode == DT_THUMBTABLE_MODE_FILEMANAGER) {
      ...
      dt_thumbnail_t *first = table->list->data;
      // clamp the movement to ensure we don't go before the first image or
      // after last one
      const int max_up =
          ((first->rowid - 1) / table->thumbs_per_row) * table->thumb_size -
          table->thumbs_area.y;
      posy = MIN(posy, max_up);

      // nb of line of the full collection
      const uint32_t nblines =
          ceilf(MAX(1, dt_collection_get_collected_count()) /
                (float)table->thumbs_per_row);
      ...
      posy =
          MAX(space - table->thumbs_area.y - table->thumbs_area.height, posy);
  ...
  if (table->mode == DT_THUMBTABLE_MODE_FILEMANAGER) {
    ...
    table->offset =
        MAX(1, table->offset -
                   (ceilf((posy + old_areay) / (float)table->thumb_size) *
...
static void _zoomable_zoom(dt_thumbtable_t *table, const int oldzoom,
                           const int newzoom) {
  ...
  while (l) {
    ...
    // in some case the thumbnail may be out of sight. This will be handled
    // later.
    gtk_layout_move(GTK_LAYOUT(table->widget), th->w_main, th->x, th->y);
    l = g_list_next(l);
  ...
  const int space =
      new_size * 0.5; // we want at least 1/2 thumb to stay visible
...
static void _filemanager_zoom(dt_thumbtable_t *table, const int oldzoom,
                              const int newzoom) {
  // nothing to do if thumbtable is empty
  if (!table->list)
    return;
  ...
  if (!thumb) {
    ...
    if (thumb) {
      ...
      if (!thumb) {
        ...
        // chained dereference is dangerous, but there was a check above in the
        // code
...
void dt_thumbtable_zoom_changed(dt_thumbtable_t *table, const int oldzoom,
                                const int newzoom) {
  if (oldzoom == newzoom)
    return;
...
static gboolean _event_scroll_compressed(gpointer user_data) {
  ...
  if (table->scroll_value != 0) {
    ...
    // starting from here, all further scroll event will count for the next
    // round
...
static gboolean _event_scroll(GtkWidget *widget, GdkEvent *event,
  ...
  if (table->mode == DT_THUMBTABLE_MODE_FILEMANAGER &&
    ...
    if (did_scroll) {
      ...
      if (table->scroll_timeout_id == 0) {
        table->scroll_timeout_id =
            g_timeout_add(10, _event_scroll_compressed, table);
      }
  ...
  if (dt_gui_get_scroll_unit_deltas(e, &delta_x, &delta_y)) {
    ...
    if (table->mode == DT_THUMBTABLE_MODE_ZOOM ||
        dt_modifier_is(e->state, GDK_CONTROL_MASK)) {
      if (table->mode == DT_THUMBTABLE_MODE_FILMSTRIP) {
        const int sx = CLAMP(table->view_width /
                                 ((table->view_width / table->thumb_size / 2 +
                                   (delta_x + delta_y)) *
                                      2 +
                                  1),
...
// display help text in the center view if there's no image to show
static void _lighttable_expose_empty(cairo_t *cr, const int32_t width,
  ...
  // 1. Draw "No Images" Header
  pango_font_description_set_size(desc, 24 * PANGO_SCALE);
  pango_font_description_set_weight(desc, PANGO_WEIGHT_BOLD);
  pango_layout_set_font_description(layout, desc);
  pango_layout_set_alignment(layout, PANGO_ALIGN_CENTER);
  pango_layout_set_width(
      layout, width * PANGO_SCALE); // Full width for center alignment

  gchar *header_text = g_strdup_printf(_("No images found"));
  pango_layout_set_text(layout, header_text, -1);
  g_free(header_text);
  ...
  cairo_move_to(cr, 0, center_y - 100);
  ...
  gchar *sub_text =
      g_strdup_printf(_("Import images to start editing your library."));
  pango_layout_set_text(layout, sub_text, -1);
  g_free(sub_text);

  pango_layout_get_pixel_extents(layout, &ink, &logical);
  dt_gui_gtk_set_source_rgba(cr, DT_GUI_COLOR_LIGHTTABLE_FONT, 0.7);
  cairo_move_to(cr, 0, center_y - 100 + 40);
  pango_cairo_show_layout(cr, layout);

  if (lighttable) {
    // 3. Draw "Import" Button
    const int btn_w = 200;
    const int btn_h = 50;
    const int btn_x = center_x - (btn_w / 2);
    const int btn_y = center_y + 20;

    // Button Background
    cairo_new_path(cr);
    // Rounded corners
    double radius = 5.0;
    cairo_arc(cr, btn_x + radius, btn_y + radius, radius, M_PI, 3 * M_PI / 2);
    cairo_arc(cr, btn_x + btn_w - radius, btn_y + radius, radius, 3 * M_PI / 2,
              0);
    cairo_arc(cr, btn_x + btn_w - radius, btn_y + btn_h - radius, radius, 0,
              M_PI / 2);
    cairo_arc(cr, btn_x + radius, btn_y + btn_h - radius, radius, M_PI / 2,
              M_PI);
    cairo_close_path(cr);

    // Using selected thumbnail BG for nice accent
    ...
    pango_layout_set_font_description(layout, desc);
    pango_layout_set_alignment(layout, PANGO_ALIGN_CENTER);
    pango_layout_set_width(layout, btn_w * PANGO_SCALE);
    pango_layout_set_text(layout, _("Import"), -1);

    pango_layout_get_pixel_extents(layout, &ink, &logical);
    // Center vertically within button
    double text_y = btn_y + (btn_h - (logical.height / PANGO_SCALE)) / 2.0;
    cairo_move_to(cr, btn_x, text_y);

    // Use theme-defined selected font color
    dt_gui_gtk_set_source_rgb(cr, DT_GUI_COLOR_THUMBNAIL_SELECTED_FONT);
    pango_cairo_show_layout(cr, layout);

    // Update click target (reusing manual_button for "Import" action)
    lighttable->manual_button.x = btn_x;
    lighttable->manual_button.y = btn_y;
    lighttable->manual_button.width = btn_w;
    lighttable->manual_button.height = btn_h;
  }
...
static gboolean _event_draw(GtkWidget *widget, cairo_t *cr,
                            dt_thumbtable_t *table) {
  if (!GTK_IS_CONTAINER(gtk_widget_get_parent(widget)))
    return TRUE;

  // we render the background (can be visible if before first image / after last
  // image)
  ...
  if (!darktable.collection ||
    ...
    GtkAllocation allocation;
    gtk_widget_get_allocation(table->widget, &allocation);
    _lighttable_expose_empty(
        cr, allocation.width, allocation.height,
...
static gboolean _event_leave_notify(GtkWidget *widget, GdkEventCrossing *event,
  ...
  if (event->detail == GDK_NOTIFY_INFERIOR ||
      event->mode == GDK_CROSSING_GTK_GRAB || event->mode == GDK_CROSSING_GRAB)
    return FALSE;
...
static gboolean _event_enter_notify(GtkWidget *widget, GdkEventCrossing *event,
  ...
  // we only handle the case where we enter thumbtable from an inferior (a
  // thumbnail) this is when the mouse enter an "empty" area of thumbtable
  if (event->detail != GDK_NOTIFY_INFERIOR)
    return FALSE;
...
static gboolean _event_button_press(GtkWidget *widget, GdkEventButton *event,
  ...
  if (table->mode != DT_THUMBTABLE_MODE_ZOOM && !dt_is_valid_imgid(id) &&
    ...
    if (event->x >= button->x && event->x <= button->x + button->width &&
      ...
      dt_lib_module_t *mod = dt_lib_get_module("import");
      if (mod) {
        dt_ui_panel_show(darktable.gui->ui, DT_UI_PANEL_LEFT, TRUE, FALSE);
        if (mod->expander) {
          dtgtk_expander_set_expanded(DTGTK_EXPANDER(mod->expander), TRUE);
        }
      }
    }
...
static void _thumbs_ask_for_discard(dt_thumbtable_t *table) {
  ...
  if (min_level < max_level) {
    ...
    if (max_level >= DT_MIPMAP_8 && min_level == DT_MIPMAP_0)
      dt_util_str_cat(&txt,
                      _("all cached thumbnails need to be invalidated.\n\n"));
    else if (max_level >= DT_MIPMAP_8)
      dt_util_str_cat(&txt,
                      _("cached thumbnails starting from level %d need to be "
                        "invalidated.\n\n"),
                      min_level);
      ...
      dt_util_str_cat(&txt,
                      _("cached thumbnails between level %d and %d need to be "
                        "invalidated.\n\n"),
                      min_level, max_level);

    dt_util_str_cat(&txt, _("do you want to do that now?"));

    if (dt_gui_show_yes_no_dialog(_("cached thumbnails invalidation"), "", "%s",
                                  txt)) {
      DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                  "SELECT id FROM main.images", -1, &stmt,
                                  NULL);
      while (sqlite3_step(stmt) == SQLITE_ROW) {
  ...
  // in any case, we update thumbtable prefs values to new ones and update
  // backthumbs database
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                              "UPDATE main.images"
                              " SET thumb_maxmip = ?1"
...
static void _dt_profile_change_callback(gpointer instance, const int type,
                                        dt_thumbtable_t *table) {
  if (!table)
    return;
...
static void _dt_active_images_callback(gpointer instance,
  ...
  if (!darktable.view_manager->active_images)
    return;
  const int activeid =
      GPOINTER_TO_INT(darktable.view_manager->active_images->data);
...
static void _dt_mouse_over_image_callback(gpointer instance,
  ...
  for (const GList *l = table->list; l; l = g_list_next(l)) {
    ...
    const gboolean drag_ko =
        (table->mode == DT_THUMBTABLE_MODE_ZOOM && table->dragging &&
    ...
    // during dragging, we can "lost" the drag_thumb if the pointer goes out of
    // the central view when the pointer is back, let's restore it
    if (th->imgid == imgid && table->mode == DT_THUMBTABLE_MODE_ZOOM &&
        table->dragging && table->drag_initial_imgid == imgid &&
        table->drag_thumb != th) {
      table->drag_thumb = th;
      table->drag_thumb->moved =
          ((abs(table->drag_dx) + abs(table->drag_dy)) > DT_PIXEL_APPLY_DPI(8));
...
static void _dt_collection_changed_callback(
    gpointer instance, dt_collection_change_t query_change,
    dt_collection_properties_t changed_property, gpointer imgs,
    const dt_imgid_t next, dt_thumbtable_t *table) {
  if (!table)
    return;
  ...
  if (query_change == DT_COLLECTION_CHANGE_RELOAD) {
    ...
    if (nrow <= 0 && actual) {
      ...
      for (const GList *l = g_list_previous(actual); l;
           l = g_list_previous(l)) {
        dt_thumbnail_t *thumb = l->data;
        nrow = _thumb_get_rowid(thumb->imgid);
        if (nrow > 0) {
    ...
    if (offset_changed)
      dt_view_lighttable_change_offset(darktable.view_manager, FALSE,
                                       table->offset_imgid);
    else {
    ...
    // if the previous hovered image isn't here anymore, try to hover "next"
    // image
    if (dt_is_valid_imgid(old_hover) && dt_is_valid_imgid(next)) {
      // except for darkroom when mouse is not in filmstrip (the active image
      // primes)
    ...
    const int nextpos =
        MAX(dt_conf_get_int("plugins/lighttable/collect/history_next_pos"), 1);
    ...
    dt_thumbtable_full_redraw(table, TRUE);
    dt_view_lighttable_change_offset(darktable.view_manager, TRUE,
                                     table->offset_imgid);
  }
}
    ...
    if (imgs_nb) {
      ...
      if (!imgs) {
        dt_print(DT_DEBUG_ALWAYS,
                 "[thumbtable] out of memory preparing drop target");
        break;
      }
      ...
      gtk_selection_data_set(
          selection_data, gtk_selection_data_get_target(selection_data), _DWORD,
          (guchar *)imgs, imgs_nb * sizeof(dt_imgid_t));
    }
    break;
  }
...
static void _event_dnd_begin(GtkWidget *widget, GdkDragContext *context,
                             dt_thumbtable_t *table) {
...
#endif
  {
    ...
    if (g_list_is_singleton(table->drag_list)) {
      ...
      if (buf.buf) {
        ...
        GdkPixbuf *source = gdk_pixbuf_new_from_data(
            buf.buf, GDK_COLORSPACE_RGB, TRUE, 8, buf.width, buf.height,
            buf.width * 4, NULL, NULL);
        GdkPixbuf *scaled =
            gdk_pixbuf_scale_simple(source, w, h, GDK_INTERP_HYPER);
    ...
    if (table->drag_list) {
      if (darktable.collection->params.sorts[DT_COLLECTION_SORT_CUSTOM_ORDER] &&
        ...
        dt_collection_update_query(
            darktable.collection, DT_COLLECTION_CHANGE_RELOAD,
...
static void _event_dnd_end(GtkWidget *widget, GdkDragContext *context,
                           dt_thumbtable_t *table) {
  if (table->drag_list) {
...
dt_thumbtable_t *dt_thumbtable_new() {
  ...
  table->offset =
      MAX(1, dt_conf_get_int("plugins/lighttable/collect/history_pos0"));
  ...
  gtk_drag_source_set(table->widget, GDK_BUTTON1_MASK, target_list_all,
                      n_targets_all, GDK_ACTION_MOVE);
  gtk_drag_dest_set(table->widget, GTK_DEST_DEFAULT_ALL, target_list_all,
                    n_targets_all, GDK_ACTION_MOVE);
  g_signal_connect_after(table->widget, "drag-begin",
                         G_CALLBACK(_event_dnd_begin), table);
  g_signal_connect_after(table->widget, "drag-end", G_CALLBACK(_event_dnd_end),
                         table);
  g_signal_connect(table->widget, "drag-data-get", G_CALLBACK(_event_dnd_get),
                   table);
  ...
  g_signal_connect(G_OBJECT(table->widget), "draw", G_CALLBACK(_event_draw),
                   table);
...
void dt_thumbtable_scrollbar_changed(dt_thumbtable_t *table, const float x,
                                     const float y) {
  ...
  if (table->mode == DT_THUMBTABLE_MODE_FILEMANAGER) {
    ...
    const float first_line =
        ((table->offset - 1) / table->thumbs_per_row) * table->thumb_size -
        table->thumbs_area.y;
...
void dt_thumbtable_full_redraw(dt_thumbtable_t *table, const gboolean force) {
  ...
  if (_compute_sizes(table, force)) {
    ...
    if (table->mode == DT_THUMBTABLE_MODE_ZOOM) {
      ...
      // (i.e. we don't get empty spaces in the very first row)
      offset =
          (table->offset - 1) / table->thumbs_per_row * table->thumbs_per_row +
          1;

      // ensure that we don't go up too far (we only want a space <thumb_size at
      // the bottom)
      if (table->offset != offset && offset > 1 && table->thumbs_per_row > 1) {
        ...
        const int move = (table->rows - 1) -
                         ((nb - (offset - 1) + table->thumbs_per_row - 1) /
                          table->thumbs_per_row);
        if (move > 0)
    ...
    // let's create a hashtable of table->list in order to speddup search in
    // next loop
    GHashTable *htable = g_hash_table_new_full(g_int_hash, g_int_equal, NULL,
                                               _list_remove_thumb);
    for (const GList *l = table->list; l; l = g_list_next(l)) {
      dt_thumbnail_t *th = l->data;
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
      ...
      dt_thumbnail_t *thumb =
          (dt_thumbnail_t *)g_hash_table_lookup(htable, &nid);
      ...
      if (thumb) {
        ...
        thumb = dt_thumbnail_new(table->thumb_size, table->thumb_size,
                                 IMG_TO_FIT, nid, nrow, table->overlays,
    ...
    if (darktable.view_manager->active_images &&
      ...
      // css effect
      const dt_imgid_t lastid = GPOINTER_TO_INT(
          g_slist_last(darktable.view_manager->active_images)->data);
      dt_thumbtable_ensure_imgid_visibility(table, lastid);

      for (GSList *l = darktable.view_manager->active_images; l;
           l = g_slist_next(l)) {
        dt_thumbnail_t *th =
            _thumbtable_get_thumb(table, GPOINTER_TO_INT(l->data));
        if (th) {
...
// change thumbtable parent widget. Typically from center screen to filmstrip
// lib
...
// get current offset
int dt_thumbtable_get_offset(dt_thumbtable_t *table) { return table->offset; }
// set offset and redraw if needed
gboolean dt_thumbtable_set_offset(dt_thumbtable_t *table, const int offset,
...
static void _thumbtable_init_accels() {
  ...
  dt_action_register(thumb_actions, N_("copy history"), _accel_copy, GDK_KEY_c,
                     GDK_CONTROL_MASK);
  dt_action_register(thumb_actions, N_("copy history parts"), _accel_copy_parts,
                     GDK_KEY_c, GDK_CONTROL_MASK | GDK_SHIFT_MASK);
  dt_action_register(thumb_actions, N_("paste history"), _accel_paste,
                     GDK_KEY_v, GDK_CONTROL_MASK);
  dt_action_register(thumb_actions, N_("paste history parts"),
                     _accel_paste_parts, GDK_KEY_v,
                     GDK_CONTROL_MASK | GDK_SHIFT_MASK);
  dt_action_register(thumb_actions, N_("discard history"), _accel_hist_discard,
                     0, 0);

  dt_action_register(thumb_actions, N_("duplicate image"), _accel_duplicate,
                     GDK_KEY_d, GDK_CONTROL_MASK);
  dt_action_register(thumb_actions, N_("duplicate image virgin"),
                     _accel_duplicate, GDK_KEY_d,
                     GDK_CONTROL_MASK | GDK_SHIFT_MASK);

  /* setup selection accelerators */
  dt_action_register(thumb_actions, N_("select all"), _accel_select_all,
                     GDK_KEY_a, GDK_CONTROL_MASK);
  dt_action_register(thumb_actions, N_("select none"), _accel_select_none,
  ...
  dt_action_register(thumb_actions, N_("select film roll"), _accel_select_film,
                     0, 0);
...
static gboolean _filemanager_key_move(dt_thumbtable_t *table,
  ...
  // collection
  if (!first_move || move == DT_THUMBTABLE_MOVE_PAGEUP ||
      move == DT_THUMBTABLE_MOVE_PAGEDOWN || move == DT_THUMBTABLE_MOVE_START ||
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(
        ...
        "SELECT MAX(rowid) FROM memory.collected_images", -1, &stmt, NULL);
    if (sqlite3_step(stmt) == SQLITE_ROW)
...
static gboolean _zoomable_key_move(dt_thumbtable_t *table,
  ...
  // direct start/end
    ...
    // this is NOT a noop due to rounding...
    moved = _move(table, newx - table->thumbs_area.x,
                  newy - table->thumbs_area.y, TRUE);
  }
...
gboolean dt_thumbtable_reset_first_offset(dt_thumbtable_t *table) {
  ...
  dt_thumbnail_t *first = table->list->data;
  const int offset =
      table->thumbs_per_row - ((first->rowid - 1) % table->thumbs_per_row);
  if (offset == 0)
    return FALSE;
