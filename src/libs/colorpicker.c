...
const gchar *dt_lib_colorpicker_model_names[] = {
    N_("RGB"), N_("RGB%"), N_("Lab"),  N_("LCh"), N_("HSL"),
    N_("HSV"), N_("Hex"),  N_("none"), NULL};
const gchar *dt_lib_colorpicker_statistic_names[] = {N_("mean"), N_("min"),
                                                     N_("max"), NULL};
...
const char *name(dt_lib_module_t *self) { return _("color picker"); }
...
int expandable(dt_lib_module_t *self) { return 1; }

int position(const dt_lib_module_t *self) { return 800; }
...
static gboolean _sample_draw_callback(GtkWidget *widget, cairo_t *cr,
  ...
  if (sample->copied) {
    ...
    if (icon_width > 0 && icon_height > 0) {
      ...
      gdk_cairo_set_source_rgba(cr, &fg_color);
      dtgtk_cairo_paint_store(cr, border, border, icon_width, icon_height, 0,
                              NULL);
    }
  }
...
static void _update_sample_label(dt_lib_module_t *self,
  ...
  case DT_LIB_COLORPICKER_MODEL_RGB:
    snprintf(text, sizeof(text), "%6d %6d %6d", sample->label_rgb[0],
             sample->label_rgb[1], sample->label_rgb[2]);
    break;
  ...
  case DT_LIB_COLORPICKER_MODEL_HSL:
    ...
    snprintf(text, sizeof(text), "%6.02f %6.02f %6.02f", alt[0] * 360.f,
             alt[1] * 100.f, alt[2] * 100.f);
    break;
...
static gboolean _large_patch_toggle(GtkWidget *widget, GdkEvent *event,
                                    dt_lib_colorpicker_t *data) {
  const gboolean show_large_patch =
      !dt_conf_get_bool("ui_last/colorpicker_large");
...
static void _setup_sample(dt_lib_module_t *self, const gboolean denoise,
...
static gboolean
_sample_tooltip_callback(GtkWidget *widget, const gint x, const gint y,
  ...
  for (int i = 0; i < DT_PICK_N; i++) {
    ...
    sample_parts[i + 4] =
        g_strdup_printf("<span foreground='#FF7F7F'>%6d</span>  "
    ...
    sample_parts[i + 8] = g_strdup_printf(
        "%6.02f  %6.02f  %6.02f  %s", sample->lab[i][0], sample->lab[i][1],
...
static gboolean _sample_leave_callback(GtkWidget *widget, GdkEvent *event,
                                       gpointer data) {
...
static void _remove_sample(dt_colorpicker_sample_t *sample) {
  ...
  darktable.lib->proxy.colorpicker.live_samples = g_slist_remove(
      darktable.lib->proxy.colorpicker.live_samples, (gpointer)sample);
  free(sample);
}
...
static gboolean _live_sample_button(GtkWidget *widget, GdkEventButton *event,
  ...
  if (event->button == GDK_BUTTON_PRIMARY) {
    ...
    if (data->target_sample) {
      ...
      memcpy(&sample->point, &data->primary_sample.point,
             sizeof(data->primary_sample.point));
      memcpy(&sample->box, &data->primary_sample.box,
...
static void _add_sample(GtkButton *widget, dt_lib_module_t *self) {
  ...
  GtkWidget *color_patch_wrapper =
      dt_gui_hbox(dt_gui_expand(sample->color_patch));
  ...
  gtk_label_set_ellipsize(GTK_LABEL(sample->output_label),
  ...
  GtkWidget *delete_button =
      dtgtk_togglebutton_new(dtgtk_cairo_paint_remove, 0, NULL);
...
void gui_init(dt_lib_module_t *self) {
  ...
  dt_lib_colorpicker_t *data = self->data =
      calloc(1, sizeof(dt_lib_colorpicker_t));
  ...
  gtk_widget_set_tooltip_text(color_patch,
                              _("click to (un)hide large color patch"));
  ...
  data->statistic_selector = dt_bauhaus_combobox_new_full(
      DT_ACTION(self), NULL, N_("statistic"),
      _("select which statistic to show"), data->statistic,
      (GtkCallback)_statistic_changed, self,
      dt_lib_colorpicker_statistic_names);
  dt_bauhaus_combobox_set_entries_ellipsis(data->statistic_selector,
  ...
  data->color_mode_selector = dt_bauhaus_combobox_new_full(
      DT_ACTION(self), NULL, N_("color mode"),
      _("select which color mode to use"), data->model,
      (GtkCallback)_color_mode_changed, self, dt_lib_colorpicker_model_names);
  dt_bauhaus_combobox_set_entries_ellipsis(data->color_mode_selector,
                                           PANGO_ELLIPSIZE_NONE);
  ...
  data->picker_button =
      dt_color_picker_new(NULL, DT_COLOR_PICKER_POINT_AREA, picker_row);
  gtk_widget_set_tooltip_text(
      data->picker_button,
  ...
  gtk_widget_add_events(sample_row_events,
                        GDK_ENTER_NOTIFY_MASK | GDK_LEAVE_NOTIFY_MASK);
  ...
  g_signal_connect(G_OBJECT(label), "size-allocate",
                   G_CALLBACK(_label_size_allocate_callback),
                   &data->primary_sample);

  data->add_sample_button =
      dtgtk_button_new(dtgtk_cairo_paint_square_plus, 0, NULL);
  ...
  gtk_container_add(GTK_CONTAINER(sample_row_events),
                    dt_gui_hbox(sample_patch_wrapper, dt_gui_expand(label),
                                data->add_sample_button));

  data->samples_container = dt_gui_vbox();

  data->display_samples_check_box = gtk_check_button_new_with_label(
      _("display samples on image/vectorscope"));
  ...
  gboolean restrict_histogram =
      dt_conf_get_bool("ui_last/colorpicker_restrict_histogram");
