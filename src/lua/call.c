...
/*
   ...
   * threads are saved in the table at REGISTRY_INDEX "dt_lua_bg_thread", that
  table is manipulated with save_thread, get_thread, drop_thread. They have a
  unique integer ID
   * each thread is a lua_State with the following convention for its stack
      ...
      * int : nresults
      * void * : callback data
      * dt_lua_finish_callback : called when the thread is finished, with or
  without errors
      -- bottom of the stack --
...
static void run_async_thread_main(gpointer data, gpointer user_data) {
  ...
  if (!thread) {
    dt_print(DT_DEBUG_LUA,
             "LUA ERROR : no thread found, this should never happen");
    return;
  }
...
static void run_async_thread(lua_State *L, int thread_num) {
  g_thread_pool_push(darktable.lua_state.pool, GINT_TO_POINTER(thread_num),
                     NULL);
  // lock ownership is transferred to the new thread. We want to block until it
  // is returned to us either because the other thread finished or because it
  // paused
  dt_lua_lock();
}
...
static gboolean end_job_dispatch(GSource *source, GSourceFunc callback,
                                 gpointer user_data) {
...
static gboolean stacked_job_dispatch(GSource *source, GSourceFunc callback,
                                     gpointer user_data) {
  gpointer message;
...
/*
   ALIEN JOB
   This is a source that deals with lua jobs that are not on the stack but
   passed as varags. This is used to queue a lua job without owning the lua lock
   */
...
static gboolean alien_job_dispatch(GSource *source, GSourceFunc callback,
                                   gpointer user_data) {
  gpointer message;
  ...
  while (cur_elt) {
    ...
    case LUA_ASYNC_TYPEID_WITH_FREE:
      ...
      // do not break
    case LUA_ASYNC_TYPEID:
      luaA_push_type(new_thread, GPOINTER_TO_INT(type_elt->data),
                     data_elt->data);
      break;
    case LUA_ASYNC_TYPENAME_WITH_FREE:
      // skip the destructor
      ...
      // do not break
    case LUA_ASYNC_TYPENAME:
      luaA_push_type(new_thread, luaA_type_find(L, type_elt->data),
                     &data_elt->data);
      break;
    case LUA_ASYNC_DONE:
    default:
...
static gboolean string_job_dispatch(GSource *source, GSourceFunc callback,
                                    gpointer user_data) {
  gpointer message;
...
static void string_job_finalize(GSource *source) {
  gpointer message =
      g_async_queue_try_pop(darktable.lua_state.string_job_queue);
  while (message) {
...
void dt_lua_async_call_internal(const char *function, int line, lua_State *L,
  ...
  g_async_queue_push(darktable.lua_state.stacked_job_queue,
                     GINT_TO_POINTER(reference));
  g_main_context_wakeup(darktable.lua_state.context);
}

void dt_lua_async_call_alien_internal(const char *call_function, int line,
                                      lua_CFunction pusher, int nresults,
...
#ifdef _DEBUG
  dt_print(DT_DEBUG_LUA, "LUA DEBUG : %s called from %s %d", __FUNCTION__,
           call_function, line);
#endif
  ...
  while (cur_type != LUA_ASYNC_DONE) {
    ...
    case LUA_ASYNC_TYPEID:
      data->extra =
          g_list_append(data->extra, GINT_TO_POINTER(va_arg(ap, luaA_Type)));
...
void dt_lua_async_call_string_internal(const char *function, int line,
                                       const char *lua_string, int nresults,
                                       dt_lua_finish_callback cb,
                                       void *cb_data) {
#ifdef _DEBUG
...
static gpointer lua_thread_main(gpointer data) {
  darktable.lua_state.pool =
      g_thread_pool_new(run_async_thread_main, NULL, -1, false, NULL);
  darktable.lua_state.loop =
      g_main_loop_new(darktable.lua_state.context, false);
...
gboolean dt_lua_gtk_wrap_callback(gpointer data) {
  ...
  lua_pushcfunction(communication->L, create_backtrace);
  // Insert at index 1 (bottom of stack)
  lua_insert(communication->L, 1);

  // Call with error handler at index 1
  communication->retval = lua_pcall(
      communication->L, lua_gettop(communication->L) - 2, LUA_MULTRET, 1);

  // Remove the traceback function from position 1
  lua_remove(communication->L, 1);
...
static int gtk_wrap(lua_State *L) {
  ...
  if (pthread_equal(darktable.control->gui_thread, pthread_self())) {
    ...
    communication.L = L;
    g_mutex_lock(&communication.end_mutex);
    g_main_context_invoke_full(NULL, G_PRIORITY_HIGH_IDLE,
                               dt_lua_gtk_wrap_callback, &communication, NULL);
    ...
    dt_print(DT_DEBUG_LUA, "LUA DEBUG : %s return for call from from %s %llu",
             __FUNCTION__, lua_tostring(L, lua_upvalueindex(2)),
             lua_tointeger(L, lua_upvalueindex(3)));
#endif
