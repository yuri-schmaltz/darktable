...
void dt_lua_debug_table_internal(lua_State *L, int t, const char *function,
                                 int line) {
  t = lua_absindex(L, t);
...
void dt_lua_init_lock() {
  ...
  // we want our lock initialized locked so that code between
  // dt_lua_init_early() and dt_lua_init() can't use lua
...
void dt_lua_lock_internal(const char *function, const char *file, int line,
                          gboolean silent) {
#ifdef _DEBUG
  if (!silent && !darktable.lua_state.ending &&
      pthread_equal(darktable.control->gui_thread, pthread_self()) != 0) {
    dt_print(DT_DEBUG_LUA,
             "LUA WARNING locking from the gui thread should be avoided");
    // g_assert(false);
  }
